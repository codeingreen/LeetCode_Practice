...
class Solution:
    def subarraySum(self, nums: List[int], k: int) -> int:
        ...
        for num in nums:
            ...
            # need to check if sum  -k is already in 
            # whenever the sums has increased by a value of k, 
            # we've found a subarray of sums=k.
            if s-k in d:
                count += d[s-k]
                #print('adding to count ', s-k, d[s-k], count)
            
            #print(num, s, s-k, count, d)
            # if s already in d, then increase by 1, if not assign 1
            if s in d:
                d[s] +=1
            else:
                d[s] =1
        
        # return only the occurance
        return count
